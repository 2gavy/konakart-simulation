...
<% if (kkEng != null) { %>
    ...
    <% 
    com.konakart.appif.CustomerIf currentCustomer = kkEng.getCustomerMgr().getCurrentCustomer();
    if (currentCustomer != null && currentCustomer.getFirstName() != null) {
%>
    <!-- If the customer is logged in and the first name is not null, display personalized content -->
    <p>Welcome, <%= currentCustomer.getFirstName() %>!</p>
<% } else { %>
    <!-- If the customer is not logged in or the first name is null, display general content -->
    <p>Welcome, Guest!</p>
<% } %>
    ...
    <% } else { %>
		<html>
			<head>
				...
				<script type="application/javascript" src="<%=kkEng.getScriptBase()%>/elastic-apm-rum.umd.min.js" crossorigin></script>
				<script>
					// Function to get or generate a session ID
					function getSessionID() {
						var existingSessionID = sessionStorage.getItem("sessionId");
						if (existingSessionID == null || existingSessionID == "null") {
							const newSessionID = 'generated-session-id-' + Math.floor(Math.random() * 1000);
				...
				<script>
				if("<%=kkEng.getCustomerMgr().getCurrentCustomer().getFirstName()%>" != "null"){
					var apm = elasticApm.init({
					serviceName: 'konakart-rum',
					serverUrl: 'https://xxx.apm.asia-southeast1.gcp.elastic-cloud.com:443',
					serviceVersion: "1.0.0",
					distributedTracingOrigins: ['*']
					});
					
					apm.setUserContext({
					'username': "<%=kkEng.getCustomerMgr().getCurrentCustomer().getFirstName()%>",
					'id': "<%=kkEng.getCustomerMgr().getCurrentCustomer().getId()%>",
					'email': "<%=kkEng.getCustomerMgr().getCurrentCustomer().getEmailAddr()%>"
					});
					apm.addLabels({"sessionId": getSessionID()})
